...
def lstm():
    ...
    model.fit(train_x, train_y, batch_size=300, epochs=100, validation_split=0.1, callbacks=[early_stopping])
...
def getBitcoin(step):
    ...
    step_time = step * 60 * 60
    price_and_time = {}
    while start_time < int(time.time()):
        price = btc_chart_data.iloc[
            btc_chart_data.index.get_loc(start_time, method='nearest')]['high']
